...
def evaluate_fixed_ppo(episodes=20):
    def make_env():
    ...
    model = PPO.load("models/ppo_fixed", env=env)
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            if done_array[0]:
                ...
                # DummyVecEnv puts original info in terminal_observation sometimes,
                # but 'episode_summary' is part of info[0] directly returned from the env step
                # if the env added it before returning. Let's check info[0].
